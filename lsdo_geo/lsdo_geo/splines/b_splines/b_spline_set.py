...
class BSplineSet(m3l.Function):
    ...
    def project(self, points:np.ndarray, targets:list[str]=None, direction:np.ndarray=None,
        ...
        if isinstance(direction.shape[0], int): # checks if only one direction vector is given
        ...
        if long_name_space in name_space_dict.keys():
            ...
            Path("stored_files/projections").mkdir(parents=True, exist_ok=True)
            parametric_coordinates = self._project(points=points, targets=targets, direction=direction,
                grid_search_density_parameter=grid_search_density_parameter, 
                max_iterations=max_iterations, plot=plot)
    ...
    def _project(self, points:np.ndarray, targets:list[str]=None, direction:np.ndarray=None,
                grid_search_density_parameter:int=10, max_iterations=100, plot:bool=False) -> list[str,np.ndarray]:
    ...
    def plot(self, b_splines:list[str]=None, point_types:list=['evaluated_points'], plot_types:list=['surface'],
        ...
        if show:
            ...
            plotter = vedo.Plotter()
            plotter.show(plotting_elements, f'Geometry: {self.name}', axes=0, viewup="z", interactive=True)
            # plotter.show(plotting_elements, f'Geometry: {self.name}', axes=1, viewup="z", interactive=True)
            return plotting_elements
        else:
            return plotting_elements
