...
class Geometry(BSplineSet):
    ...
    def plot_meshes(self, meshes:list[m3l.Variable], mesh_plot_types:list[str]=['wireframe'], mesh_opacity:float=1., mesh_color:str='#003366',
                b_splines:list[str]=None, b_splines_plot_types:list[str]=['surface'], b_splines_opacity:float=0.25, b_splines_color:str='#E3B04B',
  ...
  mesh_color:str='#F5F0E6',
b_splines_color:str='#00629B'
        ...
        for mesh in meshes:
            ...
            if 'point_cloud' in mesh_plot_types:
                ...
                plotting_elements.append(vedo.Points(points.reshape((num_points,-1)), r=4).color('#C8102E'))
            ...
            if ('surface' in mesh_plot_types or 'wireframe' in mesh_plot_types) and len(points.shape) == 3: # If it's a surface
                ...
                plotting_mesh = vedo.Mesh([vertices, faces]).opacity(mesh_opacity).color('lightyellow')
        ...
        if show:
            ...
            plotter.show(plotting_elements, 'Meshes', axes=0, viewup="z", interactive=True, camera=camera)
