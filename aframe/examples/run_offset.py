...
n = 21
mesh_a = np.zeros((n,3))
mesh_a[:,1] = np.linspace(-10,10,n)

mesh_b = np.zeros((n,3))
mesh_b[:,1] = 5
mesh_b[:,0] = np.linspace(-3,3,n)
mesh_b[:,2] = -0.5
...
class Run(csdl.Model):
    ...
    def define(self):
        ...
        self.create_input('wing_mesh', shape=(n,3), val=mesh_a)
        self.create_input('wing_t', shape=(n - 1), val=0.001)
        self.create_input('wing_r', shape=(n - 1), val=0.1)
        self.create_input('wing_forces', shape=(n,3), val=forces_a)

        self.create_input('boom_mesh', shape=(n,3), val=mesh_b)
        self.create_input('boom_t', shape=(n - 1), val=0.001)
        self.create_input('boom_r', shape=(n - 1), val=0.1)
        self.create_input('boom_forces', shape=(n,3), val=forces_b)
...
if __name__ == '__main__':
    ...
    beams['boom'] = {'E': 69E9,'G': 26E9,'rho': 2700,'cs': 'tube','nodes': list(range(n))}
    bounds['root'] = {'beam': 'wing','node': 10,'fdim': [1,1,1,1,1,1]}
    joints['j1'] = {'beams': ['wing','boom'],'nodes': [15,10]}
