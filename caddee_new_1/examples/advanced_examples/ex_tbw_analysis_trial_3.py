"Working - wing VLM --> wing structural full baseline"
...
# sim = Simulator(caddee_csdl_model,name = 'beam_mass', analytics=True)
...
sizing_wing_strut_jury_individual = False # working individually
...
if sizing_wing_strut_jury_individual:
    ...
    if sizing_2_point_5g:
        ...
        # system_model.add_constraint(plus_1_point_0g_eb_beam_outputs.bot_buckling, upper=1)
        system_model.add_constraint(plus_2_point_5g_eb_beam_outputs_wing.top_buckling[0], upper=1.1)
        system_model.add_constraint(plus_2_point_5g_eb_beam_outputs_wing.displacements[0], lower=-1.2, upper=0.5)
    ...
    if sizing_minus_1_point_0g:
        ...
        system_model.add_constraint(minus_1_point_0g_eb_beam_outputs_wing.bot_buckling[0], upper=1.1)
        # system_model.add_constraint(minus_1_point_0g_eb_beam_outputs_wing.top_buckling, upper=1.3)
        # system_model.add_constraint(minus_1_point_0g_eb_beam_outputs_wing.displacements, lower=-0.5, upper=0.5)
...
sizing = True # working without displacement constraint
...
if sizing: 
    ...
    sim = Simulator(caddee_csdl_model,name = 'beam_full_outputs_trial_2', analytics=True)
    sim['plus_2_point_5g_vlm_model.plus_2_point_5g_vlm_model.VLMSolverModel.VLM_outputs.LiftDrag.evaluation_pt'] = np.array([0., 0., 2.8])
    sim['minus_1g_vlm_model.minus_1g_vlm_model.VLMSolverModel.VLM_outputs.LiftDrag.evaluation_pt'] = np.array([0., 0., 2.8])
    ...
    prob = CSDLProblem(problem_name='full_beam', simulator=sim)
    optimizer = SLSQP(prob, maxiter=200, ftol=1E-5)
    optimizer.solve()
    optimizer.print_results()
