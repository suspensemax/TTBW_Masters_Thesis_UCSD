"TBW 63 wing strut location --> span, chord, sweep and twist geometry working?"
...
vertex01_left_strut = np.array([55.105, -16.786, -3.259]) # left leading 2
vertex02_left_strut = np.array([56.670, -27.148, -0.908]) # left leading 3
vertex03_left_strut = np.array([58.667, -33.364, 0.503]) # left leading 4
vertex04_left_strut = np.array([58.549, -39.581, 1.913]) # left leading 5
vertex05_left_strut = np.array([60.115, -49.943, 4.265]) # left leading 6
vertex06_left_strut = np.array([61.798, -54.088, 5.203]) # left leading 7

vertex10_left_strut = np.array([57.309, -12.641, -4.200]) # left trailing 1
vertex11_left_strut = np.array([59.030, -16.786, -3.259]) # left trailing 2
vertex12_left_strut = np.array([60.596, -27.148, -0.908]) # left trailing 3
vertex13_left_strut = np.array([60.478, -33.364, 0.503]) # left trailing 4
vertex14_left_strut = np.array([62.475, -39.581, 1.913]) # left trailing 5
vertex15_left_strut = np.array([64.041, -49.943, 4.263]) # left trailing 6                
vertex16_left_strut = np.array([63.610, -54.088, 5.205]) # left trailing 7
...
vertex01_right_strut = np.array([55.105, 16.786, -3.259]) # right leading 2
vertex02_right_strut = np.array([56.670, 27.148, -0.908]) # right leading 3
vertex03_right_strut = np.array([58.667, 33.364, 0.503]) # right leading 4
vertex04_right_strut = np.array([58.549, 39.581, 1.913]) # right leading 5
vertex05_right_strut = np.array([60.115, 49.943, 4.265]) # right leading 6
vertex06_right_strut = np.array([61.798, 54.088, 5.203]) # right leading 7

vertex10_right_strut = np.array([57.309, 12.641, -4.200]) # right trailing 1
vertex11_right_strut = np.array([59.030, 16.786, -3.259]) # right trailing 2
vertex12_right_strut = np.array([60.596, 27.148, -0.908]) # right trailing 3
vertex13_right_strut = np.array([60.478, 33.364, 0.503]) # right trailing 4
vertex14_right_strut = np.array([62.475, 39.581, 1.913]) # right trailing 5
vertex15_right_strut = np.array([64.041, 49.943, 4.263]) # right trailing 6                
vertex16_right_strut = np.array([63.610, 54.088, 5.205]) # right trailing 7
...
left_jury_le_left = np.array([58.721, -33.359, 0.568])
left_jury_le_right = np.array([58.052, -33.359, 6.017])
left_jury_te_left = np.array([59.676, -33.359, 0.568])
left_jury_te_right = np.array([59.237, -33.359, 6.017])
...
right_jury_le_left = np.array([58.721, 33.359, 0.568])
right_jury_le_right = np.array([58.052, 33.359, 6.017])
right_jury_te_left = np.array([59.676,33.359, 0.568])
right_jury_te_right = np.array([59.237,33.359, 6.017])
...
# print(f"The angle between the points is {angle_beta:.2f} degrees.")
...
if FFD_working_old_5: 
    ...
    # left -ve y
    wing_left_strut_connection_point = np.array([62.687, -54.094, 5.352])
    ...
    wing_right_strut_connection_point = np.array([62.687, 54.094, 5.352])
    ...
    # left
    wing_left_jury_connection_point = np.array([58.633, -33.359, 6.017])
    ...
    # right 
    wing_right_jury_connection_point = np.array([58.633, 33.359, 6.017])
    ...
    # left
    left_jury_left_strut_connection_point = np.array([59.190, -33.359, 0.568])
    ...
    #right
    right_jury_right_strut_connection_point = np.array([59.190, 33.359, 0.568])
...
FFD_working_old_6 = False # span and chord and twist and sweep
...
#     wing_fuselage_connection_input = system_model.create_input(name='wing_fuselage_connection', shape=(3, ), val=wing_fuselage_connection_m3l.value)
    ...
    wing_sweep_angle = system_model.create_input(name='wing_sweep_angle', shape=(1,), val=np.deg2rad(13.17), dv_flag = geometry_dv, 
    ...
    left_strut_sweep_coefficients[np.array([0])] = (-1/1.75) * wing_sweep_coefficients[np.array([0])]
    ...
    right_strut_sweep_coefficients[np.array([1])] = (-1/1.75) *  wing_sweep_coefficients[np.array([2])]
    ...
    left_jury_translation_x_coefficients[np.array([0])] = left_strut_sweep_coefficients[np.array([0])]/(1.8)
    ...
    right_jury_translation_x_coefficients[np.array([0])] = right_strut_sweep_coefficients[np.array([1])]/(1.8)
...
FFD_working_old_7 = True # span and chord and twist 
