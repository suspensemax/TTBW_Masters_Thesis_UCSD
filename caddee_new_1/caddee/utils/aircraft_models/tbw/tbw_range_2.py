...
class Tbw_range_Model(m3l.ExplicitOperation):
    ...
    def evaluate(self, L_over_D: Tbw_L_over_D, wing_beam_mass_props: m3l.Variable, tbw_mass_properties:m3l.Variable ,thrust: m3l.Variable,)->Tbw_range:
...
class Tbw_range_Model_CSDL(ModuleCSDL):
    ...
    def define(self):
        ...
        # c_1g = self.declare_variable('c1g', shape=(num_nodes, 1), val = 0.51535 * 10**(-3)) #TSFC
        c_1g = self.declare_variable('c1g', shape=(num_nodes, 1), val = 0.03) #TSFC
        ...
        total_mass_props = wing_beam_mass_props + tbw_mass_properties
        self.print_var(total_mass_props)
        ...
        constnt_try = self.declare_variable('constnt_try', shape=(num_nodes, 1), val = 1)
        x = weight_ratio - constnt_try
        # self.print_var(weight_ratio)
        # log_weight_ratio = csdl.log(weight_ratio)
        # log_weight_ratio = x - (x**2)/2 + (x**3)/3 - 
        # result = self.declare_variable('result', shape=(num_nodes, 1), val = 0)
        
        # for n in range(1, 100 + 1):
        #     kkk = self.declare_variable('kkk', shape=(num_nodes, 1), val = ((-1) ** (n + 1)))
        #     n_1 = self.declare_variable('n_1', shape=(num_nodes, 1), val = n)
        #     term = kkk * (x ** n) / n_1
        #     result = result + term

        log_weight_ratio = x - (x**2)/2 + (x**3)/3 - (x**4)/4 + (x**5)/5
        self.register_output(name = 'log_weight_ratio', var = log_weight_ratio)
        self.register_output(name = 'weight_ratio', var = weight_ratio)

        c_11g = self.declare_variable('c_11g', shape=(num_nodes, 1), val = 0.02 * 10**(-3)) #TSFC
        ...
        range_cruise_m3l = self.create_output(name='range_cruise_m3l', shape=(num_nodes,1), val=0)
        range_cruise_m3l[:,0] = range_a
