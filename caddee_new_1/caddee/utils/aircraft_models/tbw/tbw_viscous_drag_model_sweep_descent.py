...
class Tbw_Viscous_Drag_Model(m3l.ExplicitOperation):
    ...
    def evaluate(self, atmos: AtmosphericProperties, ac_states : AcStates, 
                 ...
                 mach_number:Union[m3l.Variable, None],angle_beta:Union[m3l.Variable, None],)->Tbwviscousdrag:
    # def evaluate(self, atmos: AtmosphericProperties, ac_states : AcStates, 
    #              tbw_area_outputs_plus_1_point_0g: geometryOutputs, chord: Union[m3l.Variable, None], vlm_outputs: VLMOutputs, 
...
class Tbw_Viscous_Drag_Model_CSDL(ModuleCSDL):
    def initialize(self):
        ...
        # self.parameters.declare(name='strut_reference_area', default=20.49)  # 220.58 ft^2 = 20.49 m^2
        self.parameters.declare(name='strut_reference_area', default=23.76)  # 220.58 ft^2 = 20.49 m^2
        # self.parameters.declare(name='strut_reference_area', default=26.80)  # 288.44713 ft^2 = 26.80 m^2
    ...
    def define(self):
        ...
        cd_interfernce = (constant_3/(reference_area_m2 + 2 * strut_reference_area_m2)) * (17*(t_by_c**4) - 0.05*(t_by_c**2) + 0.1*(cl_vlm**2) - 0.000018* (sweep_angle_expand**2) 
                                                                                       + 0.00009 * sweep_angle_expand + 0.000006 * angle_beta**2 + 0.0015*angle_beta)
        ...
        # Cf = ( 0.664 / (Re)**0.5 ) * 0 + (self.parameters['wing_viscous_cf'])*area_fraction + cd_interfernce + cd_wave #drag coefficients from tbw report M70-GE
        Cf = ( 0.664 / (Re)**0.5 ) * 0 + (self.parameters['wing_viscous_cf'])*area_fraction*0.85 + cd_interfernce + cd_wave*0.4 #drag coefficients from tbw report M70-GE
        # Cf = ( 0.664 / (Re)**0.5 ) * 0 + (self.parameters['wing_interference_cf'])*area_fraction + cd_viscous + cd_wave #drag coefficients from tbw report M70-GE
        ...
        cd_interfernce = cd_interfernce * area_fraction
        Cf_viscous_1 = self.parameters['wing_interference_cf'] * area_fraction
        ...
        self.register_output(name='Cf_viscous_1', var=Cf_viscous_1)
        self.register_output(name='Cf_interference', var=cd_interfernce)
        ...
        self.register_output(name='sweep_angle_expand', var=sweep_angle_expand)
