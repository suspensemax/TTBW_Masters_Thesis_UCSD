...
class RotationalBlockDeformationsCSDL(csdl.Model):
    ...
    def define(self):
        ...
        affine_deformed_control_points = self.declare_variable('affine_deformed_ffd_control_points', val=ffd_set.affine_deformed_control_points)
        ...
        if ffd_set.num_dof != 0:
            rotated_control_points = self.create_output('rotated_ffd_control_points', shape=(ffd_set.num_control_points,NUM_PARAMETRIC_DIMENSIONS))
        ...
        for ffd_block in list(ffd_set.active_ffd_blocks.values()):
            ...
            affine_control_points_local_frame_reshaped = csdl.reshape(ffd_block_affine_deformed_control_points, ffd_block.primitive.shape)
            ...
            for i in range(ffd_block.num_sections):
                ...
                affine_control_points_local_frame_without_translations_flattened = csdl.reshape(affine_control_points_local_frame_without_translations[i,:,:,:], 
...
if __name__ == "__main__":
    ...
    print('Python evaluation: rotational deformed FFD control_points: \n', rotated_ffd_control_points)

    sim = Simulator(RotationalBlockDeformationsCSDL(ffd_set=ffd_set))
    sim.run()
    # sim.visualize_implementation()        # Only csdl_om can do this

    print('CSDL evaluation: rotational deformed FFD control_points: \n', sim['rotated_ffd_control_points'])
    print("Python and CSDL difference", np.linalg.norm(sim['rotated_ffd_control_points'] - rotated_ffd_control_points))

    wing_ffd_block.plot_sections(control_points=sim['rotated_ffd_control_points'].reshape(wing_ffd_bspline_volume.shape), offset_sections=True, plot_embedded_entities=False, opacity=0.75, show=True)
    wing_ffd_block.plot_sections(control_points=rotated_ffd_control_points.reshape(wing_ffd_bspline_volume.shape), offset_sections=True, plot_embedded_entities=False, opacity=0.75, show=True)
    ...
    wing_ffd_block.plot_sections(control_points=(sim['rotated_ffd_control_points'][0:11*2*2,:]).reshape(wing_ffd_bspline_volume.shape), offset_sections=True, plot_embedded_entities=False, opacity=0.75, show=True)
    horizontal_stabilizer_ffd_block.plot_sections(control_points=(sim['rotated_ffd_control_points'][11*2*2:,:]).reshape(horizontal_stabilizer_ffd_bspline_volume.shape), offset_sections=True, plot_embedded_entities=False, opacity=0.75, show=True)
